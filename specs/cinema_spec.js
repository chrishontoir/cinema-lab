...
describe('Cinema', function () {
  ...
  it('should be able to find a film by title', function () {
    const actual = cinema.filmFindByTitle("Moonlight");
    assert.strictEqual(actual, moonlight);
  });

  it('should be able to filter films by genre', function () {
    const actual = cinema.filmFindByGenre("action");
    assert.deepStrictEqual(actual, [blackPanther]);
  });

  it('should be able to check whether there are some films from a particular year', function () {
    const actual = cinema.filmFindByYear(2017);
    assert.strictEqual(actual, true);
  });

  it('should be able to check whether there are no films from a particular year', function () {
    const actual = cinema.filmFindByYear(2000);
